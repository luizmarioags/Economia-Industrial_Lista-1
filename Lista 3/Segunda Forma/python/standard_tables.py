...
def nested_shares(price_vec: np.ndarray, base_df: pd.DataFrame, coef: dict) -> np.ndarray:
    ...
    alpha = float(coef["alpha"])
    scale = max(1 - sigma, 1e-8)
    delta = (
        coef["b0"]
        + coef["bcals"] * base_df["cals"].to_numpy(float)
        + coef["bfat"] * base_df["fat"].to_numpy(float)
        + coef["bsugar"] * base_df["sugar"].to_numpy(float)
        - alpha * price_vec
    )
...
def main():
    ...
    export_csv_tex(
        all_coef,
        config.TABCSV / "01_all_coefficients.csv",
        config.TABTEX / "01_all_coefficients.tex",
    ...
    for name, m in models.items():
        alpha = _alpha_from_model(m)
        se_alpha = m.se.get("alpha", m.se.get("bp", m.se.get("price", np.nan)))
        rows.append({
            "model": name,
            "price_coef_minus_alpha": -alpha if np.isfinite(alpha) else np.nan,
            "alpha": alpha,
            "alpha_se": se_alpha,
            "gmm_objective": m.Q,
            "sigma_nested": m.coefficients.get("sigma", np.nan),
        })
    price_parameter = pd.DataFrame(rows)
    export_csv_tex(
        price_parameter,
        config.TABCSV / "02_price_parameter_comparison.csv",
        config.TABTEX / "02_price_parameter_comparison.tex",
        "Comparação do parâmetro de preço",
        "tab:price-compare",
    )
    ...
    rows_df["row_id"] += 1
    cols_df["col_id"] += 1
    simple = rows_df[["row_id", "product", "price", "share"]].merge(
        cols_df[["col_id", "product", "price", "share"]],
        how="cross",
        suffixes=("_row", "_col"),
    )
    simple["elasticity"] = np.where(
        simple["row_id"] == simple["col_id"],
        -alpha * simple["price_row"] * (1 - simple["share_row"]),
        alpha * simple["price_col"] * simple["share_col"],
    )
    ...
    simple_sub = rows_sub[["row_id", "product", "price", "share"]].merge(
        cols_sub[["col_id", "product", "price", "share"]],
        how="cross",
        suffixes=("_row", "_col"),
    )
    simple_sub["elasticity"] = np.where(
        simple_sub["row_id"] == simple_sub["col_id"],
        -alpha * simple_sub["price_row"] * (1 - simple_sub["share_row"]),
        alpha * simple_sub["price_col"] * simple_sub["share_col"],
    )
