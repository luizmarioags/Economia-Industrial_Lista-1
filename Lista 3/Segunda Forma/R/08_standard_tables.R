...
price_parameter <- purrr::imap_dfr(models, function(m, nm) {
  ...
  se_price <- if ("bp" %in% names(se)) se["bp"] else if ("price" %in% names(se)) se["price"] else NA_real_
  sigma <- if ("sigma" %in% names(b)) b["sigma"] else NA_real_
  tibble::tibble(
    model = nm,
    price_coef_minus_alpha = as.numeric(coef_price),
    alpha = as.numeric(-coef_price),
    price_se = as.numeric(se_price),
    gmm_objective = m$Q,
...
export_csv_tex(elas_simple, file.path(TABCSV, "03_elasticity_matrix_simple_logit.csv"), file.path(TABTEX, "03_elasticity_matrix_simple_logit.tex"), "Matriz de elasticidades - logit simples", "tab:elas-simple")
...
elas_simple_sub <- tidyr::crossing(rows_sub, cols_sub) %>%
  mutate(elasticity = ifelse(row_id == col_id,
                             ...
                             alpha * column_price * column_share)) %>%
  select(row_product, column_product, elasticity)
export_csv_tex(elas_simple_sub, file.path(TABCSV, "04_elasticity_matrix_simple_logit_subset.csv"), file.path(TABTEX, "04_elasticity_matrix_simple_logit_subset.tex"), "Matriz de elasticidades - logit simples - subconjunto", "tab:elas-simple-subset")

# 05. Elasticidades próprias simples
own_simple <- df %>% mutate(own_elasticity_simple_logit = -alpha*price*(1-share)) %>% arrange(desc(share)) %>%
  select(product, firm, segment, price, share, own_elasticity_simple_logit)
export_csv_tex(own_simple, file.path(TABCSV, "05_own_elasticities_simple_logit.csv"), file.path(TABTEX, "05_own_elasticities_simple_logit.tex"), "Elasticidades próprias - logit simples", "tab:own-elas-simple")

# Funções do nested logit
nested_shares <- function(price_vec, base_df, coef) {
  sigma <- as.numeric(coef["sigma"])
  scale <- max(1 - sigma, 1e-8)
  delta <- as.numeric(coef["b0"] + coef["bcals"]*base_df$cals + coef["bfat"]*base_df$fat + coef["bsugar"]*base_df$sugar + coef["bp"]*price_vec)
...
) %>% mutate(elasticity = E_nested[cbind(row_id, col_id)]) %>% select(row_product, column_product, elasticity)
export_csv_tex(elas_nested, file.path(TABCSV, "06_elasticity_matrix_nested_logit.csv"), file.path(TABTEX, "06_elasticity_matrix_nested_logit.tex"), "Matriz de elasticidades - nested logit", "tab:elas-nested")

own_nested <- df %>% transmute(product, own_elasticity_nested_logit = diag(E_nested)) %>% arrange(own_elasticity_nested_logit)
export_csv_tex(own_nested, file.path(TABCSV, "07_own_elasticities_nested_logit.csv"), file.path(TABTEX, "07_own_elasticities_nested_logit.tex"), "Elasticidades próprias - nested logit", "tab:own-elas-nested")

# 08. Markups
s <- df$share; f <- df$idfirm; Delta <- matrix(0, n, n)
for (j in seq_len(n)) for (k in seq_len(n)) if (f[j] == f[k]) Delta[j,k] <- if (j == k) alpha*s[j]*(1-s[j]) else -alpha*s[j]*s[k]
...
) %>% select(product, firm, segment, price, share, markup_monoproduct, mc_monoproduct, markup_multiproduct, mc_multiproduct, markup_mono_over_price, markup_multi_over_price)
export_csv_tex(markups, file.path(TABCSV, "08_markups.csv"), file.path(TABTEX, "08_markups.tex"), "Markups implícitos - logit simples", "tab:markups")
...
fs <- readRDS(fs_path)
export_csv_tex(fs, file.path(TABCSV, "09_first_stage_diagnostics.csv"), file.path(TABTEX, "09_first_stage_diagnostics.tex"), "Diagnóstico de primeiro estágio", "tab:first-stage")
