...
first_stage_diag <- tibble::tibble(
  endogenous_variable = character(), excluded_instruments = numeric(),
  partial_F_homoskedastic = numeric(), robust_Wald_F_manual = numeric(),
  first_stage_R2 = numeric(), specification = character()
)
...
add_diag <- function(y, excluded, spec) {
  ...
  # Aproximação: usando o mesmo Wald-F robusto como métrica operacional do pacote.
  tibble::tibble(
...
first_stage_diag <- bind_rows(
  first_stage_diag,
  add_diag("price", ZBOTH, "simple_logit_both"),
  add_diag("price", ZNESTALL, "nested_logit"),
